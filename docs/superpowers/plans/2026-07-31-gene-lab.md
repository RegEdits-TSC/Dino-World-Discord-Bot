...
**Interfaces:**
- Consumes: nothing.
- Produces: `TraitId`, `TraitDomain`, `TraitDef`, `TraitMods`, `TRAITS`, `TRAIT_IDS`, `getTrait(id: string): TraitDef`, `traitDefs(ids: string[]): TraitDef[]`, `modProduct(ids: string[], key: keyof TraitMods): number`, `WILD_SLOT_ODDS`, `BRED_SLOT_ODDS`, `rollTraits(rng: () => number, odds?: readonly number[]): TraitId[]`, `pickTrait(rng: () => number, exclude: Set<TraitDomain>): TraitId | null`, `spliceTrait(current: string[], slot: number, rng: () => number): TraitId[]`.
...
function rollSlotCount(odds: readonly number[], rng: () => number): number {
  const r = rng();
  let acc = 0;
...
import { modProduct, pickTrait, TRAITS, BRED_SLOT_ODDS, type TraitDomain, type TraitId } from '../../data/traits.js';
import {
  BREED_MS, BREED_FEE, BREED_COOLDOWN_MS, BREED_UPGRADE_CHANCE,
...
export function inheritTraits(parentA: string[], parentB: string[], rng: () => number): string[] {
  ...
  let count = 0;
  let acc = 0;
  for (let i = 0; i < BRED_SLOT_ODDS.length; i++) {
    acc += BRED_SLOT_ODDS[i];
    if (r < acc) { count = i; break; }
    count = BRED_SLOT_ODDS.length - 1;
  }
...
export function claimPayload(opts: { rarity: string; traits: string[]; upgraded: boolean; speciesName: string | null }): Payload {
  const embed = new EmbedBuilder().setColor(0x9b59b6)
    .setTitle('🧬 A new egg!')
      ...
      async execute(ctx, i) {
        ...
        try {
          if (sub === 'start') {
            ...
            const ready = activeBreedings(ctx, i.user.id).find((b) => b.readyAt <= ctx.now());
            if (!ready) {
              await i.reply({ content: 'Nothing to claim — no breeding has finished.', flags: MessageFlags.Ephemeral });
              return;
            }
            const { egg, upgraded } = claimBreeding(ctx, i.user.id, ready.id);
