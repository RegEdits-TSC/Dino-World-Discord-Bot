...
// SUPERSEDED DURING EXECUTION: this originally took `{ skipLockCheck?: boolean }`
// and substituted empty maps. That waived EVERY lock reason, not just the
// accept-time self-lock, so a dino held by both a trade and an unclaimed breeding
// would transfer mid-breed. The waiver is scoped to the accepting trade instead.
export function verifySide(ctx: Ctx, userId: string, side: TradeSide, opts: { forTradeId?: number } = {}): void {
  ...
  const escrowed = (r?: LockReason) => r !== undefined && !(r.kind === 'trade' && r.tradeId === opts.forTradeId);
...
it('rolls fresh traits for a wild egg and stores them on the dino', () => {
  ...
  // Must pin a NON-EMPTY result for this seed, or the test cannot tell a working
  // roll from `const traits = egg.traits.length ? egg.traits : []` — both give [].
  // Verify the seed produces traits, then assert the exact values it produces.
  expect(dino.traits.length).toBeGreaterThan(0);
});
...
export function hatchEgg(ctx: Ctx, userId: string, eggId: number): { species: Species; dinoId: number; traits: string[] } {
  ...
  // CORRECTED DURING EXECUTION: discriminate on `source`, not on array length. A bred
  // egg legitimately inherits ZERO traits 25% of the time under BRED_SLOT_ODDS, and an
  // empty array is indistinguishable from a wild egg's — so the length check re-rolled
  // those on wild odds and silently shipped a bred distribution of
  // [13.75%, 53.75%, 32.5%] instead of the intended [25%, 45%, 30%].
...
Add a `gene_lab` entry to each of the three hand-maintained kind maps: the fallback glyph in `src/data/render-icons.ts`, the icon lookup in `src/core/render/art.ts`, and the label/emoji map in `src/modules/park/embeds.ts`. Match the shape each file already uses for `hatchery_lab`.

**Do NOT add `dw_lot_genelab` to `EMOJI_FALLBACK` here.** `tests/emoji-assets.test.ts:103` asserts a strict bidirectional equality between the SVG files on disk and the fallback-table keys, so a key without its SVG turns the suite red. Task 12 adds the key and authors the SVG in the same commit, which is the pairing that test exists to enforce. Until then a Gene Lab lot renders via the `render-icons.ts` glyph, which is the designed degrade path.

**Naming:** park lot icons follow the `dw_lot_*` convention (`dw_lot_hatchery`, `dw_lot_visitor`, `dw_lot_food_court` at `src/core/emojis.ts:14-15`). The Gene Lab icon is therefore **`dw_lot_genelab`**, not `dw_gene_lab` — Task 12 registers it as `dw_lot_genelab: '🧬'`.
